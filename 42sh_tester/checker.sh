...
test_shells()
{
	...
	do
		...
		then
			...
			if [ $success -eq 1 ]
			then
				echo -e " -> match with [\033[32m$shell\033[0m]"
				let "TOTAL_OK++"
			else
				echo -e " \033[31m unmatch \033[0m"
			fi
			break
		fi
	done
...
new_test()
{
	...
	dir_files_to_test=$1
	echo -e "\033[36mDir : " $dir_files_to_test/ "\033[0m"
